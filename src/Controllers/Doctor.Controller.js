const { convertToISOTime, filterdetail } = require("../../Constants");
...
const { getData, addData } = require("../Utils/dataModule");
...
exports.setCriteria = asyncHandler(async (req, res) => {
  const { HowManyPatients, day, start, end, date } = req.body;
  ...
  addData(day, scheduledTime?.date, res);
  ...
  if (data.length > 0) {
    ...
    return res.status(500).json({ error: "array is empty" });
  }
