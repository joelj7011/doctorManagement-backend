    ...
    try {
        ...
        if (data !== null) {
            try {
                ...
                if (!parseDate) {
                    return res.status(403).json({ error: "could not parse the data at:", details: { location: __filename, lLine: getLineNumber() } });
                } else {
                    console.log("test2->passed");
                }

                const decryptedDate = decrypt(parseDate?.encryptedData, parseDate?.iv, res);
                if (!decryptedDate) {
                    return res.status(403).json({ error: "could not decrypt  the data  at :", details: { location: __filename, lLine: getLineNumber() } });
                } else {
                    return res.status(200).json(JSON.parse(decryptedDate));
                }
            } catch (decryptErr) {
                return res.status(500).json({ error: `Decryption failed: ${decryptErr.message}`, details: { location: __filename, lLine: getLineNumber() } });
            }
        } else {
...
exports.setCahe = asyncHandler(async (req, res, next) => {
    ...
    if (!iv && encryptedData) {
        ...
        return res.status(500).json({ error: "Could not encrypt the data at:", details: { location: __filename, lLine: getLineNumber() } });
    } else {
        console.log("test1->passed");
    }

    const cacheData = await client.setEx(`user:${req.user.id}`, process.env.REDIS_CACHE_EXPIRY, JSON.stringify({
        iv, encryptedData
    }));
    if (!cacheData) {
        console.log("Data could not be cached");
        return res.status(500).json({ error: "Data could not be cached", details: { location: __filename, lLine: getLineNumber() } });
    } else {
        console.log("test2->passed");
    }
    next();
